...
class PolicyNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_size, action_range=1., init_w=3e-3, log_std_min=-20, log_std_max=2):
        ...
        self.lstm = nn.LSTM(input_size=num_inputs, hidden_size=hidden_size, num_layers=5)#(input_size,hidden_size,num_layers)
        # input = torch.randn(5, 3, 10)#(seq_len, batch, input_size)
        # h0 = torch.randn(2, 3, 20) #(num_layers,batch,output_size)
        # c0 = torch.randn(2, 3, 20) #(num_layers,batch,output_size)
        # output, (hn, cn) = rnn(input, (h0, c0))
        # ————————————————
        # 版权声明：本文为CSDN博主「HxShine」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
        # 原文链接：https://blog.csdn.net/qq_16949707/article/details/84845552
    ...
    def forward(self, state):
        ...
        # x = self.tcn(state)
        # state = state.reshape(state_seq_len, -1, input_channels)
        state = state.view(state_seq_len, -1, input_channels)
        hx, cx = self.lstm(state)
        x = hx
        # print(x.shape)
        # input()
        # x = x.transpose(1, 2)
    ...
    if args.train:
        ...
        for eps in range(max_episodes):
            ...
            for step in range(max_steps):
                ...
                # print(next_state.shape, reward.shape, action.shape, done)
                # input()
                # env.render()       
